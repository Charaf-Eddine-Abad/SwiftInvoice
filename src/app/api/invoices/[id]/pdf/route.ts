...
export async function GET(
  ...
  const errorDetails = { step: 'init', detail: '' }
  try {
...
async function generateInvoiceHTML(invoice: InvoiceData) {
  try {
    ...
    const subtotal = invoice.invoiceItems.reduce((sum: number, item: InvoiceItem) => {
        ...
        <div class="overflow-hidden border border-gray-200 rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ${Array.isArray(invoice.invoiceItems) ? invoice.invoiceItems.map((item: InvoiceItem) => `
